...
func (shop *serverShop) UpdateCustomer() http.HandlerFunc{
	...
	myLogger.InfoLoggerFunc(customer.FirstName)

	db.Model(&Customer{}).Where("id=?",id).Updates(map[string]interface{}{
...
func (shop *serverShop) DeleteCustomer() http.HandlerFunc{
	...
	var customer Customer
	myLogger.InfoLoggerFunc(string(id))

	db.Model(&Customer{}).Delete(&customer,id)
